import { Home, Gift, Ticket, User, Settings } from 'lucide-react';
...
export function BottomNav({ currentScreen, onNavigate, isAdmin }: BottomNavProps) {
  ...
  const isActive = (screen: string) => {
    ...
    if (screen === 'admin' && (currentScreen === 'admin' || currentScreen === 'admin-sorteo')) return true;
    return false;
  };

  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-pollo-marron-claro/20 safe-area-bottom z-40">
          ...
          return (
            ...
            >
              <div className={`relative ${active ? 'transform -translate-y-1' : ''}`}>
                <Icon className={`w-6 h-6 transition-all duration-300 ${active ? 'stroke-[2.5px]' : 'stroke-2'}`} />
                {active && (
